...
flowchart LR
    ...
    B --> C["Đọc chi tiết nhiều đoạn<br/>30 phút"]
    C --> D["Highlight và ghi chú<br/>10 phút"]
    D --> E["Tìm lại ý cần dùng<br/>10 phút"]
    E --> F["Viết summary / rút insight<br/>10 phút"]

    classDef bottleneck fill:#ffe3e3,stroke:#d64545,stroke-width:2px,color:#111;
    class C,D,E,F bottleneck;
```
...
```mermaid
flowchart LR
    A["Rule: xác định mục tiêu đọc<br/>câu hỏi cần trả lời<br/>3 phút"] --> B["Workflow: upload/paste tài liệu<br/>và metadata<br/>2 phút"]
    B --> C["AI: tạo overview<br/>key points + sections + terms<br/>5 phút"]
    C --> D["AI: screening theo câu hỏi<br/>trích ý + vị trí nguồn<br/>10 phút"]
    D --> E["Người đọc kiểm chứng<br/>đoạn nguồn quan trọng<br/>15 phút"]
    E --> F["Viết summary/decision note<br/>5 phút"]
    E -. "AI thiếu/sai context" .-> G["Fallback:<br/>đọc trực tiếp phần nguồn<br/>và sửa thủ công"]

    classDef human fill:#e8f3ff,stroke:#2f6fed,stroke-width:2px,color:#111;
    classDef rule fill:#e9f7ef,stroke:#238636,stroke-width:1.5px,color:#111;
    classDef ai fill:#f2e8ff,stroke:#6f42c1,stroke-width:1.5px,color:#111;
